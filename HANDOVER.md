...
- **题目分析器** (`papercarator/planner/analyzer.py`)
  - 基于关键词匹配识别论文类型
  - 支持：optimization, differential, equation_system, network_flow, time_series, multi_objective, pde, queueing, markov_chain, game_theory, control_theory, clustering 等类型
...
- **数学建模模块** (`papercarator/math_modeling/`)
  ...
  - ✅ 已补齐 `game_theory`（博弈论）、`control_theory`（控制理论）与 `clustering`（聚类分析）的分析、写作画像、图表与回归测试链路
...
#### 0. 最新测试基线已确认
**结果**: 当前会话已运行 `python3.12 -m pytest tests -q`

**基线**: `78 passed`

**补充验证**:
- `python3.12 -m pytest tests\test_skill_assets.py tests\test_cli.py -q` 通过，`9 passed`
- `quick_validate.py papercarator` 通过，输出 `Skill is valid!`
- `python3.12 -m papercarator.cli analyze "基于优化理论的资源配置问题研究"` 输出 `paper_type=optimization`
...
#### 3. 工作树存在未提交改动
**文件**:
- `papercarator/math_modeling/model_builder.py`
- `papercarator/math_modeling/solver.py`
- `papercarator/visualization/chart_generator.py`

**说明**: 这些改动不是本轮维护新增，后续继续推进时应避免无意覆盖
...
4. [x] 增加 CLI 组装层回归测试
5. [ ] 增加 CLI 真实运行 / 产物级验证
6. [ ] 增加 PDF/LaTeX 产物级验证
7. [ ] 继续清理 Windows / MiKTeX 环境差异问题

### P2 - 可选集成
8. [ ] 修复 MATLAB Engine 安装
9. [ ] 启用 GitHub 自动发布
10. [ ] 评估 SolidWorks 集成是否保留为可选桥接

### P3 - 向 Direction B 过渡
11. [ ] 文献检索与引用管理
12. [ ] 更强的开放域题目理解
13. [ ] 更明确的工程级 3D/CAD 边界
...
- `SKILL.md` 已补齐标准 YAML frontmatter，便于作为 Codex skill 被识别
- `tests/test_skill_assets.py` 已新增，防止 skill frontmatter、调用协议和本地化预设退化
- `game_theory`、`control_theory`、`clustering` 已纳入 high-confidence model families，不再作为 generic `modeling` 降级路径处理
- 对不确定题目先运行 `python3.12 -m papercarator.cli analyze "<topic>"`
- 只有识别到稳定模型族，或明确接受“建模近似”时，才运行完整生成
- 默认完整运行继续使用 `--no-github --no-vscode`，避免非必要外部副作用
- 验收时检查 `paper.tex`、`paper.pdf`、图表、预期 3D 图，以及章节内容是否贴合模型类型
- `review`、`experimental`、`theoretical`、泛化 `modeling` 题目必须明确标注为降级/近似模式
...
**最后更新**: 2026-04-29
**当前版本**: 0.2.0
**完成度**: ~82%（基础功能可用，skill 调用契约更清晰，三类半接入模型已补齐到全链路，最新全量测试已确认；维护重点转向产物级回归和数学建模能力继续深化）
